...
import { CheckIcon, ChevronIcon, FlagIcon, QuestionIcon, WarningIcon } from "./icons";
...
/**
 * One result. Collapsed: a single row with the verdict and confidence.
 * Expanded: the question, the probability strip or option breakdown, the
 * flag, and the evidence sentence. Issues start open; passes start closed.
 */
export function AxisResultCard({ result, threshold, index = 0, expanded, onToggle }: Props) {
  ...
  const bodyId = `result-body-${result.axis.id}`;
    ...
    >
      ...
      >
        ...
        <span className="min-w-0 flex-1">
          <span className="block truncate text-sm font-medium text-ink">{result.axis.name}</span>
          <span className="block truncate text-xs text-ink-2">
            {result.verdictLabel}
            {result.verdictDetail && <span className="text-muted"> · {result.verdictDetail}</span>}
          </span>
        </span>
        {flagged && <FlagIcon className="h-3.5 w-3.5 shrink-0 text-pink" title="Low confidence" />}
        <span className={`shrink-0 font-mono text-xs tabular-nums ${result.needsReview ? "text-muted" : confidenceTone}`}>
          {result.needsReview ? "—" : formatPercent(result.confidence)}
        </span>
        <ChevronIcon className={`h-3.5 w-3.5 shrink-0 text-muted transition-[rotate] duration-150 ${expanded ? "rotate-180" : ""}`} />
      </button>

      {expanded && (
        <div id={bodyId} className="space-y-3 border-t border-line px-4 py-3">
          <p className="text-xs leading-relaxed text-muted">
            <span className="font-mono text-[10px] uppercase tracking-[0.14em]">Asked · </span>
            {result.axis.question}
          </p>
